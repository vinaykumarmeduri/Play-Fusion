...
function FreefireTournaments() {
  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/freefire-tournaments');
        if (!response.ok) {
  ...
  const handleJoinNow = async (tournamentName) => {
    try {
      ...
      toast.success('Joined successfully!', {
        onClose: () => navigate(`/chat/${response.data.name}`) // Navigate to the chat group after the toast closes
      });
    } catch (error) {
        ...
        <Container>
          <Row className="justify-content-center align-items-center vh-100">
            <Col md={8} className="text-center">
              <h2>Existing Freefire Tournaments</h2>
              {error ? (
