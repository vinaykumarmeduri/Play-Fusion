...
function WCC2Tournaments() {
  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/wcc2-tournaments');
        if (!response.ok) {
  ...
  const handleJoinNow = async (tournamentName) => {
    try {
      const username = localStorage.getItem('name'); // Get username from local storage
      if (!username) {
      ...
      // Redirect to chat group page after a delay
      setTimeout(() => {
        window.location.href = `/chat/${encodeURIComponent(response.data.name)}`;
      }, 1500);
    } catch (error) {
        ...
        <Container>
          <Row className="justify-content-center align-items-center vh-100">
            <Col md={8} className="text-center">
              <h2>Existing WCC2 Tournaments</h2>
              {error ? (
                ...
                tournaments.map((tournament) => (
                  <Card key={tournament._id} bg="dark" text="white" className="tournament-card mt-3">
                    <Card.Body>
                      ...
                      <Button variant="primary" onClick={() => handleJoinNow(tournament.tournamentName)}>Join Now</Button> {/* Add Join Now button */}
                    </Card.Body>
                  </Card>
                ))
              )}
...
export default WCC2Tournaments;
