...
function LudoKingTournaments() {
  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/ludo-king-tournaments');
        if (!response.ok) {
        ...
        <Container>
          <Row className="justify-content-center align-items-center vh-100">
            <Col md={8} className="text-center">
              <h2>Existing LudoKing Tournaments</h2>
              {error ? (
                  ...
                  <Card key={tournament._id} bg="dark" text="white" className="tournament-card mt-3">
                    <Card.Body>
                      ...
                      <Card.Text>
                        ...
                        <strong>Type of Tournament:</strong> {tournament.typeOfTournament || 'N/A'}<br />
...
export default LudoKingTournaments;
